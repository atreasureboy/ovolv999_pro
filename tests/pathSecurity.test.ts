...
describe('isPathWithin', () => {
  it('allows paths within base directory', () => {
    expect(isPathWithin(join(homedir(), 'projects', 'file.ts'), join(homedir(), 'projects'))).toBe(true)
  })
  ...
  it('rejects traversal paths', () => {
    expect(isPathWithin(join(homedir(), 'projects', '..', '..', 'etc'), join(homedir(), 'projects'))).toBe(false)
  })
})