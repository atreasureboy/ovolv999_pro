...
\`\`\``,
      parameters: {
        ...
        properties: {
          ...
          key: {
            type: 'string',
            description:
              '(keys) tmux 特殊按键名：C-c / C-d / Escape / Enter / Up / Down / Tab。支持多个用空格分隔',
          },
          lines: {
            type: 'number',
  ...
  async execute(input: Record<string, unknown>, _context: ToolContext): Promise<ToolResult> {
    switch (String(input.action)) {
      case 'new':
        return this._new(input)
      case 'send':
        return this._send(input)
      case 'keys':
        return this._keys(input)
      case 'capture':
        return this._capture(input)
      case 'wait_for':
        return this._waitFor(input)
      case 'list':
        return this._list()
      case 'kill':
        return this._kill(input)
      default:
        return {
          content: `Unknown action "${str(input.action)}". Use: new | send | keys | capture | wait_for | list | kill`,
          isError: true,
        }
    }
  }
  ...
  private async _new(input: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (await sessionExists(name)) {
      return {
        content: `Session "${name}" already exists. Use capture to check its state, or kill to recreate.`,
        isError: false,
      }
    }
    ...
    try {
      ...
      return {
        content: [
          ...
          command
            ? `  TmuxSession({ action: "wait_for", session: "${name}", pattern: "..." })  # wait for prompt`
            : '',
      ...
      return {
        content: `Failed to create session "${name}": ${(e as Error).message}`,
        isError: true,
      }
    }
  }
  ...
  private async _send(input: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (!(await sessionExists(name))) {
      return {
        content: `Session "${name}" not found. Use: TmuxSession({ action: "list" })`,
        isError: true,
      }
    }
  ...
  private async _keys(input: Record<string, unknown>): Promise<ToolResult> {
    ...
    if (!key)
      return {
        content: 'Error: key is required for keys (e.g. C-c, C-d, Escape, Enter)',
        isError: true,
      }
  ...
  private async _list(): Promise<ToolResult> {
    try {
      ...
      if (!output) {
        return {
          content:
            'No active tmux sessions.\nCreate one: TmuxSession({ action: "new", session: "py", command: "python3" })',
          isError: false,
        }
      }
      ...
      return {
        content:
          'No active tmux sessions (tmux server not running).\nCreate one: TmuxSession({ action: "new", session: "py", command: "python3" })',
        isError: false,
      }
    }
  }
